...
定妆任务为全剧共享（episode=0），依赖故事概要与完整故事，不依赖单集剧本或文字分镜。出图按集增量：本批只要求当前集用到的变体，已有资产保留。关键帧按集、按分镜生成，引用已发布的全剧定妆。切换画风影响全剧定妆及所有集的预览、关键帧、视频和成片，清除当前产物的旧视觉文件引用，重新生成时使用当前所选画风。旧文件可以保留为历史，但不能晋升为新画风结果。
